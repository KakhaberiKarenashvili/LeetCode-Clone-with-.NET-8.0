...
public class RegisterUserCommandHandler : IRequestHandler<RegisterUserCommand, Unit>
{
    ...
    public async Task<Unit> Handle(RegisterUserCommand request, CancellationToken cancellationToken)
    {
        ...
        var baseRoute = $"{_emailOptions.FrontendBaseUrl}/{_emailOptions.ConfirmEmailRoute}";

        var body = await EmailTemplateProvider.GetEmailBodyAsync(
            request.Email,
            EmailTemplateProvider.EmailConfirmationTemplate,
            token,
            baseRoute);

        await _emailService.SendEmailAsync(request.Email, EmailSubjects.ConfirmEmail, body, cancellationToken);
